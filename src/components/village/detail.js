...
class villageDetail extends Component {
  ...
  componentWillMount() {
    ...
    // 查询所有度假村图片
    fetch(appConfig.URLbase + '/Dvt-reserve/product/relResortGallery/' + _this.props.village.villageSelected.resortId + '/findByResortId.do', {
      method: 'GET',
    ...
    let fetchUrl = `${appConfig.URLbase}/Dvt-reserve/product/apartment/1/0/searchSource.do?startDate=${timeConversion.timestampToxxxx(_timestamp)}&endDate=${timeConversion.timestampToxxxx((_timestamp + _stampLeave))}&resortCode=${_this.props.village.villageSelected.resortCode}`
    fetch(fetchUrl, {
      method: 'GET',
      contentType: 'application/json; charset=utf-8'
    }).then(function (response) {
      ...
      <div>
        <div>
          <Carousel 
            ...
            selectedIndex={0}
            >
            {this.state.carousel.map(function (data) {
              let _this = this
              return <img style={{
  ...
  return roomType.map(function (value, ref) {
    return <div style={{position: 'relative'}}>
      ...
      <div className={styles.Division} onClick={function () {
        fetch(
          appConfig.URLbase + '/Dvt-reserve/product/relApartmentGallery/' + value.apartmentId + '/findByApartmentId.do', {
            method: 'GET',
            contentType: 'application/json; charset=utf-8'
          }
        ).then(function (response) {
